...
function shouldShowLeadLagChart(messages: UIMessage[], messageIndex: number) {
  ...
  const prompt = messages.slice(0, messageIndex).reverse().find((message) => message.role === "user");
  if (!prompt) return false;
  const text = prompt.parts.map((part) => (part.type === "text" ? part.text : "")).join(" ").trim();
...
export function ChatWindow({
  ...
  const transmit = useCallback(async (outgoing: UIMessage[], assistantId: string) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(raw || `GIZMO returned HTTP ${response.status}`);
      }

      if (!response.ok) throw new Error(data.error || `GIZMO returned HTTP ${response.status}`);
      if (!data.text?.trim()) throw new Error("GIZMO returned no text. Try the question again.");

      setStatus("streaming");
      setMessages((current) => current.map((message) =>
        message.id === assistantId ? textMessage(assistantId, "assistant", data.text!.trim()) : message,
      ));
      setStatus("ready");
    } catch (cause) {
      const message = cause instanceof Error && cause.name === "AbortError"
        ...
        : cause instanceof Error ? cause.message : "GIZMO could not complete that transmission.";
      setError(message);
      setStatus("error");
  ...
  const handleSubmit = useCallback((message: PromptInputMessage) => {
    ...
    setMessages([...outgoing, textMessage(assistantId, "assistant", "")]);
  ...
  const awaitingResponse = status === "submitted" || (lastMessage?.role === "assistant" && !lastMessage.parts.some((part) => part.type === "text" && part.text));

  const body = useMemo(() => messages.map((message, index) => {
    const text = message.parts.map((part) => (part.type === "text" ? part.text : "")).join("");
    ...
    <div className="flex h-full min-h-0 flex-col">
      <Conversation className={cn("min-h-0 flex-1", isEmpty && "pixel-grid")}>
        <ConversationContent className="mx-auto w-full max-w-3xl px-4 py-6">
          ...
          {error ? <p className="mt-4 border-2 border-destructive bg-terminal px-3 py-2 text-xs text-destructive">{error}</p> : null}
        </ConversationContent>
        <ConversationScrollButton />
      </Conversation>
      ...
      <div className="border-t-2 border-border bg-terminal px-3 py-3 sm:px-4 sm:py-4">
        <div className="mx-auto w-full max-w-3xl">
          <PromptInput onSubmit={handleSubmit} className="gizmo-transmit pixel-frame-inset relative bg-card">
            <PromptInputTextarea ref={textareaRef} autoFocus placeholder="Ask GIZMO…" aria-label="Ask GIZMO" className="gizmo-transmit-textarea font-mono text-sm" />
            <PromptInputFooter className="gizmo-transmit-footer absolute bottom-3 right-3 z-10 flex items-center sm:bottom-4 sm:right-4">
              <PromptInputSubmit status={status} aria-label={busy ? "Stop transmission" : "Ask GIZMO"} onClick={busy ? stop : undefined} className="gizmo-transmit-button" />
            </PromptInputFooter>
          </PromptInput>
        </div>
      </div>
...
function EmptyState() {
  return (
    <div className="flex min-h-[46vh] flex-col items-center justify-center text-center">
      ...
      <p className="mt-3 max-w-sm text-xs leading-relaxed text-muted-foreground">Find the market that moves first. Trade the market that follows.</p>
    </div>
  );
}

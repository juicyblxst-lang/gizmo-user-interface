...
function referencedMarketBefore(messages: UIMessage[], messageIndex: number): MarketPair | null { for (let index = messageIndex - 1; index >= 0; index -= 1) { const message = messages[index]; if (!message || message.role !== "user") continue; const text = message.parts.map((part) => part.type === "text" ? part.text : "").join(" "); const symbol = extractPair(text); if (symbol) return marketFromSymbol(symbol); } return null; }
function shouldShowLeadLagChart(messages: UIMessage[], messageIndex: number) { const assistantMessage = messages[messageIndex]; if (!assistantMessage || assistantMessage.role !== "assistant" || !textOf(assistantMessage).trim()) return false; const prompt = messages.slice(0, messageIndex).reverse().find((message) => message.role === "user"); if (!prompt) return false; const text = prompt.parts.map((part) => part.type === "text" ? part.text : "").join(" ").trim(); return Boolean(extractPair(text)) && (isMarketVisualizationQuestion(text) || isFollowUpQuestion(text)); }
...
export function ChatWindow({ threadId, initialMessages, marketContext, onMessagesChange }: { threadId: string; initialMessages: UIMessage[]; marketContext: MarketContext | null; onMessagesChange: (id: string, messages: UIMessage[]) => void; }) {
  const textareaRef = useRef<HTMLTextAreaElement | null>(null); const abortRef = useRef<AbortController | null>(null); const submittingRef = useRef(false);
  ...
  const transmit = useCallback(async (outgoing: UIMessage[], assistantId: string) => {
    ...
    try {
      ...
      const answer = typeof data?.text === "string" ? data.text : "GIZMO returned an empty response.";

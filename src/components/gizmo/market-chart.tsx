...
export function MarketChart({ market, onClose }: { market: MarketPair; onClose: () => void }) {
    ...
    const loadHistory = async () => {
      try {
        const response = await fetch(
          `${OKX_REST}/candles?instId=${encodeURIComponent(instId)}&bar=${encodeURIComponent(timeframeConfig.bar)}&limit=80`,
          { cache: "no-store" },
        );
    ...
    const connect = () => {
      ...
      socket.onopen = () => {
        ...
        socket.send(JSON.stringify({
          op: "subscribe",
          args: [
            { channel: timeframeConfig.channel, instId },
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (message.arg?.channel === timeframeConfig.channel) {
  ...
  const chart = useMemo(() => {
    ...
    const width = 1000;
    const height = 420;
    const padding = { top: 24, right: 78, bottom: 32, left: 16 };
          ...
          <div className="pixel-frame bg-terminal">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-border px-3 py-2">
              ...
              <div className="flex items-center gap-2">
                <label htmlFor={`timeframe-${market.replace("/", "-")}`} className="text-[9px] text-muted-foreground">TIMEFRAME</label>
                <select
                  id={`timeframe-${market.replace("/", "-")}`}
                  value={timeframe}
                  onChange={(event) => setTimeframe(event.target.value as Timeframe)}
                  className="pixel-frame-inset cursor-pointer bg-background px-2 py-1 text-[9px] text-primary outline-none hover:bg-secondary focus:ring-1 focus:ring-primary"
                  aria-label={`${market} chart timeframe`}
                >
                  {TIMEFRAMES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
                </select>
                <span className="text-[9px] text-muted-foreground">{timeframe} · {candles.length} CANDLES · OKX</span>
              </div>
            </div>
            ...
            {chart && candles.length > 1 ? (
              <div className="p-3">
                <div className="relative w-full overflow-hidden">
                  <svg viewBox={`0 0 ${chart.width} ${chart.height}`} className="block h-auto w-full" preserveAspectRatio="none" role="img" aria-label={`${market} live ${timeframe} candlestick chart`}>
                ...
                <div className="mt-2 flex items-center justify-between border-t border-border pt-2 text-[9px] text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1.5" style={{ color: connected ? LIVE_GREEN : LIVE_RED }}><span className="size-1.5 animate-pulse rounded-full" style={{ backgroundColor: connected ? LIVE_GREEN : LIVE_RED }} />LIVE TICK</span>
                </div>
              </div>
            ) : (
              <div className="flex min-h-[420px] items-center justify-center">
                <div className="text-center"><p className="text-pixel text-[9px] text-primary">{error ? "RECONNECTING TO MARKET" : "LOADING MARKET"}</p><p className="mt-3 text-xs text-muted-foreground">{error ? "The live feed will reconnect automatically…" : `Loading ${market} ${timeframe} candles…`}</p></div>
              </div>
            )}
          </div>
        </div>

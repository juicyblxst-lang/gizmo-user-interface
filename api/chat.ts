...
function symbolFromText(text: string): Symbol | null { const match = text.toUpperCase().match(/\b(BTC|BITCOIN|ETH|ETHEREUM|SOL|SOLANA|XRP|DOGE|DOGECOIN|HYPE)\b/); return match ? SYMBOL_ALIASES[match[1]] : null; }
function pair(symbol: Symbol) { return `${symbol}-USDT-SWAP`; }

async function backend(path: string, timeoutMs = 20000) {
...
function naturalAnswer(symbol: Symbol, market: any, signal: any, prompt: string, hasConversation: boolean) {
  ...
  if (leadQuestion) { const relationship = typeof signal?.correlation === "number" ? `correlation ${signal.correlation}` : "no correlation value is currently recorded"; const lagText = typeof lag === "number" ? `a measured lag of ${lag}h` : "no measured lag is currently recorded"; return [`${symbol} is currently classified as ${state}. The engine shows ${relationship} and ${lagText}. Those are measured relationships, not a forecast.`,`On the lead-lag read, ${symbol} is ${state}: ${relationship}, with ${lagText}. That's the current engine evidence, not a prediction.`,`The current engine evidence has ${symbol} at ${state}; it records ${relationship} and ${lagText}. So that's what the present data supports.`,`Right now the measurable relationship is ${state}. For ${symbol}, that's ${relationship} with ${lagText}. I'm keeping that separate from any forecast.`][variant]; }
...
export default async function handler(request: Request) {
  ...
  try {
    ...
    const market = marketResult.value; const signals = signalResult.status === "fulfilled" ? signalResult.value : null; const signal = signals?.pairs?.[pair(symbol)] ?? signals?.pairs?.[symbol] ?? {};
    const hasConversation = messages.some((message: any) => message?.role === "assistant"); const answer = naturalAnswer(symbol, market, signal, latest, hasConversation);
    return new Response(JSON.stringify({ text: answer, market, signal, pair: pair(symbol) }), { status: 200, headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" } });
  } catch (error) { console.error("GIZMO chat failed:", error); return new Response(JSON.stringify({ error: error instanceof Error ? error.message : "GIZMO could not complete that transmission." }), { status: 503, headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" } }); }
}
